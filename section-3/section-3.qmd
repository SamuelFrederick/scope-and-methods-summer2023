---
title: "Section 3. Summarizing Data, If Statements, Loops"
...
## Conditional Logic in Data: If Statements

- Logical data and Subsetting

- Often, we want to check if some condition holds for data and perform operation if that condition holds
  - e.g., check if ideology is below the median, and label that person liberal if so
...
```{r}
x <- 1:10
ifelse(x<=5, "Below Median", "Above Median")
df <- df %>% 
...
- Sometimes we have more than one condition we want to evaluate
  - `condition1&condition2` checks whether condition1 AND condition2 are TRUE
  - `condition1|condition2` checks whether condition1 OR condition2 is TRUE
...
- Sometimes we want to check whether a condition is not `TRUE`
  - `!condition` means NOT condition (condition does not hold)
